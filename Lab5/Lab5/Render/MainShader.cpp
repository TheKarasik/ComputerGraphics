...
MainShader::MainShader(Renderer* renderer, IDXGISwapChain* swap_chain, Microsoft::WRL::ComPtr<ID3D11Device> device, Display32 display32) : renderer_(renderer)
{
    ...
    constant_buffer_texture = new ConstantBuffer<MaterialProperties>(renderer_);
    constant_buffer_light = new ConstantBuffer<LightDataStruct>(renderer_);

    //Делаем BackBuffer
    ID3D11Texture2D* backTex;
    HRESULT res = swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&backTex);	// __uuidof(ID3D11Texture2D)
    res = device->CreateRenderTargetView(backTex, nullptr, &rtv_main);
...
void MainShader::ProvideMeshData(Mesh* mesh)
{
    ...
    constant_buffer_light->UpdateBuffer(&light->light_data);
